...
### P1-1 评估集与评估器
- 改动：新增 33 条 golden 数据，覆盖本地问答、历史追问、歧义、拒答、图片和降级场景；新增评估脚本，输出 Top-1/Top-3 召回、忠实度、引用完整度、P95 延迟、缓存命中率和分场景指标。
- 验证：评估专项 7/7 通过；全量 70/70 单元测试通过，耗时 2.929s；评估文件语法检查通过。
